...
class ConfigLoader:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        """
        获取默认配置
        优先从 config.example.yml 读取，如果不存在则使用最小化的硬编码配置
        """
        # 尝试从 config.example.yml 加载默认配置
        example_config_path = self.config_path.parent / "config.example.yml"
        
        if example_config_path.exists():
            try:
                with open(example_config_path, 'r', encoding='utf-8') as f:
                    example_config = yaml.safe_load(f) or {}
                print(f"📋 从示例配置文件加载默认配置: {example_config_path}")
                return example_config
            except yaml.YAMLError as e:
                print(f"⚠️  示例配置文件格式错误: {e}")
                print(f"💡 使用最小化硬编码配置")
            except Exception as e:
                print(f"⚠️  无法读取示例配置文件: {e}")
                print(f"💡 使用最小化硬编码配置")
        else:
            print(f"⚠️  示例配置文件不存在: {example_config_path}")
            print(f"💡 使用最小化硬编码配置")
        
        # 最小化的硬编码配置（仅作为最后的后备方案）
        return {
            "server": {
                "host": "0.0.0.0",
                "port": 8000,
